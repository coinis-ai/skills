...
- **Anywhere we name a generation model — in ANY file — use the house models: Seedance 2.0 or newer for video** (`seedance-2.0`, `seedance-2.0-fast`), **Seedream for image** (`seedream-5.0-lite`, `seedream-4.5`, `seedream-4.0`). This covers skill prose, `COOKBOOK.md` trigger phrasings, eval scenarios, and README copy alike; a quoted *user phrasing* ("use X for the hero clip") is a model example too. Never a superseded video tier (`seedance-1.5-pro`, `seedance-1.0-pro*`) and never another vendor's model as the default/recommended option. Non-house model ids may still be **listed in a catalogue table** — that is coverage, not a recommendation. Check with a **bare-family** grep, not a versioned one — `grep -rniE '\b(veo|seedance|seedream|gemini|grok|runway|aleph|gen4|imagen|sora|kling|pika|luma|happy.?horse)\b' --include='*.md' .` — since `veo-3` misses a bare "Veo". One deliberate exception: this rule itself names the tiers it forbids. The `preview_cost` 422 illustration uses a **vendor-neutral placeholder** (e.g. `<VariantRequest>`) when it documents the *validator's structure* — it never echoes a real non-house variant. The live catalogue is still discovered at run time via the 422 probe and never hardcoded — this governs the *examples we write*, not what the agent may fire.
